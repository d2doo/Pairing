...
class ChatServiceImplTest {
    ...
    void createChatRoom() {
        ...
        Long value = chatService.createChatRoom( new ChatRoomCreateDto( joinMembers, member1.getMemberId(), "temp", 10, ChatRoomStatus.활성화, LocalDateTime.now() ) );
        assertNotNull( value );

        Optional<ChatRoom> chatRoom = chatService.findById( value );
        assertNotNull( chatRoom.get() );

        List<UserChatRoom> userChatRooms = chatRoom.get().getUserChatRooms();
        assertEquals( userChatRooms.size(), 3 );
        assertEquals( userChatRooms.get(0).getChatRoom().getChatRoomId(), chatRoom.get().getChatRoomId() );
    }
}